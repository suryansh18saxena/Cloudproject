...
def cleanup_resources(username):
    ...
    try:
        ...
        if not instance_ids:
            ...
            print("[WARN] Note: CloudTrail takes up to 15 mins to log events. If an instance was created very recently, it might be missed.")
            return

        print(f"[WARN] Found instances in history: {instance_ids}")

        # Check if they are currently running/stopped, handling errors for completely purged instances
        try:
            ...
            if 'InvalidInstanceID.NotFound' in str(e):
                print("[OK] Some instances found in CloudTrail no longer exist in EC2. No action needed.")
            else:
                print(f"[ERROR] AWS ClientError during EC2 check: {e}")
